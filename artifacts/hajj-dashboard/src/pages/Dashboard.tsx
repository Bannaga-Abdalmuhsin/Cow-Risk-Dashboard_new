...
import { Activity, AlertCircle, BarChart3, CheckCircle2, ClipboardList, FlaskConical, Home, Leaf, LogOut, Map, Radio, Settings, Zap } from "lucide-react";
...
type Tab = "home" | "overview" | "twin" | "scenarios" | "map" | "sites" | "methodology" | "settings";
...
export default function Dashboard({ onLogout }: { onLogout?: () => void }) {
  ...
  const tabs: Array<{ key: Tab; label: string; icon: ReactNode }> = [
    { key: "home", label: "Home", icon: <Home size={15} /> }, { key: "overview", label: "Analytics", icon: <BarChart3 size={15} /> }, { key: "twin", label: "Digital Twin", icon: <Activity size={15} /> },
    { key: "scenarios", label: "Scenario Lab", icon: <FlaskConical size={15} /> }, { key: "map", label: "Geo Risk", icon: <Map size={15} /> },
    { key: "sites", label: "Site Portfolio", icon: <ClipboardList size={15} /> }, { key: "methodology", label: "Model & Data", icon: <Zap size={15} /> }, { key: "settings", label: "Settings", icon: <Settings size={15} /> },
  ];
  return <div className="min-h-screen bg-background flex flex-col">
    ...
    <nav className="overflow-x-auto border-b border-border bg-card shadow-sm" aria-label="Main navigation"><div className="flex min-w-max gap-1 px-3">{tabs.map(tab => <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`flex items-center gap-1.5 border-b-2 px-4 py-3 text-xs font-semibold transition ${activeTab === tab.key ? "border-purple-600 bg-purple-50 text-purple-800" : "border-transparent text-muted-foreground hover:bg-muted/50 hover:text-foreground"}`}>{tab.icon}{tab.label}</button>)}</div></nav>
    <main className="flex-1 p-3 sm:p-4">
      {activeTab === "home" && <WebsiteHome analyses={analyses} onNavigate={destination => setActiveTab(destination)} />}
